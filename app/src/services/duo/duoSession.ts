...
import { logWorkout } from '../workouts/workouts';
...
export async function inviteParticipant(
  ...
  const fromName = session.participants[session.creatorId]?.displayName ?? 'Someone';
  ...
  await createNotification(target.id, {
    fromUserId: session.creatorId,
    fromUserName: fromName,
    title: 'Duo Workout Invite',
    body: `${fromName} invited you to join a Duo Workout: ${session.planName || 'Custom Workout'}`,
    type: 'duo_invite',
    data: {
      sessionId,
      inviteId: inviteRef.id,
      planName: session.planName || 'Custom Workout',
      exerciseCount: session.exerciseIds.length,
    }
  });
