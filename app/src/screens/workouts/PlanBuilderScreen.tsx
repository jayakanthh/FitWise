...
import type { Exercise, PlanDay } from '../../models/index';
import { createPlan, currentUserId, getExercises, getExercisesByIds, getPlan, setActivePlan, updatePlan, deletePlan, searchExercises } from '../../services/index';
...
export default function PlanBuilderScreen({
  ...
  useEffect(() => {
    ...
    (async () => {
      const plan = await getPlan(editPlanId);
      if (!plan) return;
      const uniqueIds = Array.from(new Set(plan.days.flatMap(d => d.exercises.map(e => e.exerciseId))));
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editPlanId) {
        ...
        await refresh();
        Alert.alert('Changes Saved', 'Your training plan has been updated.', [
        ...
        Alert.alert(
          'Plan Created! 🎉',
          'Would you like to set this as your active training plan?',
          [
            {
              text: 'No',
              style: 'cancel',
...
const styles = StyleSheet.create({
  ...
  deleteBtnText: { color: '#ff4d4f', fontSize: 14, fontWeight: '700' },
});
